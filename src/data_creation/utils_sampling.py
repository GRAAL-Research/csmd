...
def ngram_copy_filtering(generateds, no_copy_texts, logits, n_gram=3):
    # `generateds`: tensor (N, L_1) a list of sequences of generated tokens which cannot repeat
    # more than `n_gram` tokens from the no_copy_text
    # `no_copy_texts`: tensor (N,L_2) a list of sequences of tokens which will be used to limit
    # the logits so that generateds cannot repeat more than n_gram subsequences from this text
    # `logits`: (N,vocab_size) if the generateds would have the possibility to form a (n_gram+1)
    # copy-ngram from no_copy_texts, then make those tokens very unlikely
